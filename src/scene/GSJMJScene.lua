-- total error 20 lines ylqj/1.0.21012
...
function MJScene:setMjSpecialData()
    ...
    self.curLuaFile = 'scene.GSJMJScene'

    self.mjTypeWanFa ='gsj'

    self.RecordGameType = RecordGameType.GSJ

    self.mjGameName = '拐三角'
end

function MJScene:loadMjLogic()
    self.MJLogic = require('logic.mjgsj_logic')
end
...
function MJScene:treatResume(rtn_msg)
    ...
    for __, player in ipairs(playerinfo_list) do
        ...
        if direct == 1 then
            ...
            self.pass_gangs = player.pass_gangs

            WARN(self.pass_gangs)
...
function MJScene:createLayerMenu(room_info)
    ...
    if self.is_3dmj then
        img_bg_title:loadTexture("ui/qj_bg/3d/3d_gsj.png")
        img_bg:loadTexture(self.img_3d[self.zhuobu])
    else
        img_bg_title:loadTexture("ui/qj_bg/2d/2d_gsj.png")
    ...
    local function pengOptTreat()
        ...
        if #index_list >= 2 then
            ...
            self.pass_gangs = self.pass_gangs or {}
            local num = 0
            for _,v in pairs(self.hand_card_list[1]) do
                if self.hand_card_list[1][index_list[1]].card_id == v.card_id then
                    num = num + 1
                end
            end
            if num == 3 then
                table.insert(self.pass_gangs,self.hand_card_list[1][index_list[1]].card_id)
            end
    ...
    self.isQiXiaoDui = room_info.isQiXiaoDui

    self.is13Yao = room_info.is13Yao

    self.isYing8Zhang = room_info.isYing8Zhang
    ...
    if self.can_opt and
        ...
        for i, v in ipairs(hand_list) do
            if not self.ting_list[v] then
                ...
                local hu_list = {}
                local ting_list = self.MJLogic.CetTingCards(hands, group_list, self.wang_cards[1], self.isQiXiaoDui, self.is13Yao, self.isYing8Zhang)
                -- print('-------------------------')
                -- dump(ting_list)
                -- print('-------------------------')
                --local ting_list = {}
...
function MJScene:initResultUI(rtn_msg)
    ...
    for i, v in ipairs(rtn_msg.players) do
        ...
        if rtn_msg.host_id == v.index and rtn_msg.lianHost and rtn_msg.lianHost >= 1 and bHu then
            str = str or ''
            str = str .. ' 连庄X' .. (rtn_msg.lianHost)
        end
...
function MJScene:GetGangPai()
    ...
    for i , v in pairs(self.pass_gangs or {}) do
        for j = #tGang , 1 , - 1 do
            if tGang[j] == v then
                table.remove(tGang,j)
                break
            end
        end
    end
...
function MJScene:getHuImg()
    ...
    hu_img[self.MJLogic.HU_KANHU]  = 5-- 砍胡
    hu_img[30] = 30                     --自摸
    return hu_img
end
...
function MJScene:getWanFaStr()
    ...
    str = str .. (room_info.diFen and '底分' .. room_info.diFen .. '分\n' or '')
