...
local PDKLogic       = require("logic.PDKLogic")
...
function PDKScene:onRcvPDKGameStart(rtn_msg)
    ...
    -- 切四张牌
    if self.poke_type == 3 then
        self.qie_card_list = {}
        for i = 1, 4 do
            self.qie_card_list[i] = cc.Sprite:create("ui/dt_ddz_play/dt_ddz_play_boxsbackCard.png")
            self.qie_card_list[i]:setPosition(cc.p(qie_card_pos.x + qie_card_width * i * qie_card_scale, qie_card_pos.y))
            self.qie_card_list[i]:setScale(qie_card_scale)
    ...
    if index == 1 then
        ...
        if rtn_msg.cur_ju then
            chuSanJu = rtn_msg.cur_ju
        else
            chuSanJu = self.cur_ju
        end

        -- 红桃3当庄
        if self.host_type == 2 then
            chuSanJu = 1
        end

        -- 没有红三出其它三，没有三出四，
        if self.chu_san and chuSanJu == 1 then
            -- 红桃三
            local target  = 0x23
            local isFirst = true
            while not ht3 do
        ...
        self:runAction(cc.Sequence:create(cc.DelayTime:create(0.2), cc.CallFunc:create(function()
            if next_index == 1 then
                ...
                if self.pre_out_direct and next_index ~= self.pre_out_direct then
                    if self.bdbcd == 1 and self.baoting_img and self.baoting_img[self.next_direct] then
                        self:showNextBaoTing()
                        self.mustChuDa = true
                    end
                    self:updateHintList()
...
function PDKScene:onRcvPDKResult(rtn_msg)
    self:runAction(cc.Sequence:create(cc.DelayTime:create(0.6), cc.CallFunc:create(function()
        ...
        if rtn_msg.rest_cards and self.qie_card_list then
            for i, v in ipairs(rtn_msg.rest_cards) do
                self.qie_card_list[i].card = self:getCardById(v)
                if g_channel_id ~= 800001 then
                    self.qie_card_list[i].card:setPosition(cc.p(36, 45))
                else
                    self.qie_card_list[i].card:setPosition(cc.p(42.5, 58))
                end
                self.qie_card_list[i]:addChild(self.qie_card_list[i].card)
                self.qie_card_list[i].card:setVisible(false)
                self.qie_card_list[i].card:setScale(0.6)
                self.qie_card_list[i]:runAction(cc.Sequence:create(cc.DelayTime:create((i - 1) * 0.15), cc.OrbitCamera:create(0.3, 1, 0, 0, 180, 0, 0), cc.CallFunc:create(function()
...
function PDKScene:treatResume(rtn_msg)
    ...
    if self.is_game_start then
        ...
        if not self.is_playback then
            local cardNum = 16
            if self.poke_type and self.poke_type == 2 then
                cardNum = 15
            end
            if self.people_num == 2 then
                if self.my_index == 1 then
        ...
        if self.is_playback then
            ...
            for ii = 1, count do
                ...
                if v.cards and v.cards[ii] then
                    ...
                    self.hand_card_list[idx][ii].card:setPosition(cc.p(self.hand_card_list[idx][ii]:getContentSize().width / 2, self.hand_card_list[idx][ii]:getContentSize().height / 2))
    ...
    if self.poke_type == 3 and (rtn_msg.result_packet or has_hand_card or self.is_playback) then
        self.qie_card_list = {}
        for i = 1, 4 do
            self.qie_card_list[i] = cc.Sprite:create("ui/dt_ddz_play/dt_ddz_play_boxsbackCard.png")
            self.qie_card_list[i]:setPosition(cc.p(qie_card_pos.x + qie_card_width * i * qie_card_scale, qie_card_pos.y))
    ...
    if next_index > 0 then
        if next_index == 1 then
            local ht3 = nil
            local num = 16
            if self.poke_type == 2 then
                num = 15
            end

            local chuSanJu = rtn_msg.cur_ju

            if self.host_type == 2 then
                chuSanJu = 1
            end

            if self.chu_san and chuSanJu == 1 and rtn_msg.player_info.hand and #rtn_msg.player_info.hand >= num then
                local target  = 35
                local isFirst = true
                while not ht3 do
            ...
            if self.pre_out_direct and next_index ~= self.pre_out_direct then
                ...
                if ht3 then
                    self.mustChuSan = true
                elseif self.bdbcd == 1 and self.baoting_img and self.baoting_img[self.next_direct] then
                    self:showNextBaoTing()
                    self.mustChuDa = true
                end
    ...
    if rtn_msg.result_packet then
        ...
        if rtn_msg.result_packet.rest_cards and self.qie_card_list then
            for i, v in ipairs(rtn_msg.result_packet.rest_cards) do
                self.qie_card_list[i].card = self:getCardById(v)
                if g_channel_id ~= 800001 then
                    self.qie_card_list[i].card:setPosition(cc.p(36, 45))
                else
...
function PDKScene:createLayerMenu(room_info)
    ...
    -- 16 15 切四
    self.poke_type = room_info.poke_type
    -- 是否要先出三
    self.chu_san = room_info.chu_san == 1
    -- 赢家当庄 1
    -- 红桃三当庄 2
    -- 轮流当庄 3
    self.host_type = room_info.host_type

    if room_info.host_type ~= 2 then
        self.chu_san = true
    end
    -- 炸弹不可拆
    self.zha_chai = room_info.zha_chai or 0 -- 0 可拆  1 不可拆
    -- 飘分
    self.piaofen = room_info.piaoniao
    -- 三带一对
    self.sdyd = room_info.sdyd or 0
    -- 三A当炸弹
    self.sadzd = room_info.sadzd or 0
    -- 四带三 2
    -- 四带二 1
    -- 不可带 0
    self.zddp = room_info.zddp or 2
    -- 炸弹不算分
    self.zdbsf = room_info.zdbsf or 0
    ...
    self.share_content = "跑得快"..str
    self.share_title   = self.desk..g_game_name
...
function PDKScene:onBtnOutCardClicked()
    ...
    if not self.sel_list or #self.sel_list < 0 then
        ...
        if self.zha_chai and self.zha_chai == 1 then
            for i, v in ipairs(self.sel_list) do
                local count = 0
                local value = PDKLogic:GetLogicValue(v)
                for i, card in ipairs(self.my_cards) do
                    if value == PDKLogic:GetLogicValue(card) then
                        count = count + 1
                    end
                end
                if count == 4 and (#self.sel_list ~= 4 or
                    PDKLogic:GetLogicValue(self.sel_list[1]) ~= PDKLogic:GetLogicValue(self.sel_list[2]) or
                    PDKLogic:GetLogicValue(self.sel_list[2]) ~= PDKLogic:GetLogicValue(self.sel_list[3]) or
                    PDKLogic:GetLogicValue(self.sel_list[3]) ~= PDKLogic:GetLogicValue(self.sel_list[4]))then
                    isChaiZhaDan = true
                    break
                end
            end
            if self.zddp == 1 then
                if PDKLogic:GetCardType(self.sel_list, self.opts) == PDKLogic.CT_FOUR_TAKE_TWO then
                    isChaiZhaDan = false
                end
            elseif self.zddp == 2 then
                if PDKLogic:GetCardType(self.sel_list, self.opts) == PDKLogic.CT_FOUR_TAKE_THREE then
                    isChaiZhaDan = false
                end
            end
        end
        if isChaiZhaDan then
            commonlib.showLocalTip("当前房间不允许拆炸弹，请重新选牌！")
            for i, v in ipairs(self.hand_card_list[1]) do
                for cii, cid in ipairs(self.sel_list) do
                    if v.card_id == cid then
                        table.remove(self.sel_list, cii)
...
function PDKScene:updateHintList()
    ...
    if pre_card ~= 0 then
        ...
        PDKLogic:SearchOutCard(next_cards, last_id_list, self.hint_list, nil, self.opts)
    end
...
function PDKScene:onTiShiClicked()
    ...
    if self.mustChuDa then
        table.sort(self.hint_list, function(x, y)
            return PDKLogic:CompareCard(y, x, self.opts)
        end)
        tishi_cards = self.hint_list[1]
    end
    log(tishi_cards)
...
function PDKScene:isMustChuCard()
    ...
    if self.mustChuDa then
        ...
        -- 炸弹带牌
        -- opts.zddp or 2
        local typ = PDKLogic:GetCardType(self.sel_list, self.opts)
        -- 如果是单牌
        if typ == 1 then
            if self.sel_list and #self.sel_list == 1 then
                local maxLogicValue = 0
                for i, v in ipairs(self.my_cards) do
                    if PDKLogic:GetLogicValue(v) > maxLogicValue then
                        maxLogicValue = PDKLogic:GetLogicValue(v)
                    end
                end
                -- 必须出最大的单牌
                if maxLogicValue == PDKLogic:GetLogicValue(self.sel_list[1]) then
                    return true
                end
            end
        else
...
function PDKScene:hasBomb(data)
    ...
    while i <= count do
        ...
        -- 3A算炸弹
        if self.sadzd == 1 and sameCount == 3 and logicValue == 14 then
            return true
        end
        i = i + sameCount
    end
...
function PDKScene:initResultUI(rtn_msg)
    ...
    Record.save_new_record(self, rtn_msg, RecordGameType.PDK)
    ...
    local copy_str = os.date("切磋时间%m-%d %H:%M\n", os.time()) .. "跑得快"..self.desk.."房第"..rtn_msg.cur_ju.."局切磋详情:\n"
    for i, v in ipairs(rtn_msg.players) do
    ...
    ccui.Helper:seekWidgetByName(node, "btn-jxyx"):addTouchEventListener(function(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            ...
            for __, v in ipairs(self.qie_card_list or {}) do
                if v then
                    v:removeFromParent(true)
                end
            end
    ...
    tolua.cast(ccui.Helper:seekWidgetByName(node, "shuoming_0"), "ccui.Text"):setString("玩法: 跑得快 "..self.wanfa_str)
    ...
    for i, v in ipairs(rtn_msg.players) do
        ...
        if v.qiang_guan and v.qiang_guan == 1 then
            tolua.cast(ccui.Helper:seekWidgetByName(play, "qiangguan"), "ccui.ImageView"):setVisible(true)
        end
