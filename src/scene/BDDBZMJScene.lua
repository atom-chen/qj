...
function MJScene:setMjSpecialData()
    ...
    self.curLuaFile = 'scene.BDDBZMJScene'
    ...
    for i, v in ipairs(rtn_msg.players) do
       ...
       if bLeastOneHu or jiesan_detail then
        ...
        if bHu then
            str = str or ''
            -- 点杠 胡牌类型 胡牌方式
            str = diangangStr .. str .. genzhuang_numStr .. huTypeStr .. szHuType
        else
            str = str or ''
            -- 点炮显示
            if self.isGSHZ then
                str = diangangStr .. genzhuang_numStr .. szHuType
            else
                str = diangangStr .. str .. genzhuang_numStr .. szHuType
            end
        end
