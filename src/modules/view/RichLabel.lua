...
--[[
	local params = {
		...
		text = [fontColor=f75d85 fontSize=20]hello world[/fontColor][image=wsk1.png][/image],
	}
	text 目前支持参数
			文字
			fontName  : font name
...
--[[ 公共方法
	...
	播放文字动画	    playFadeInAnim
	强制停止播放动画    playEnded
	是否在播放动画      isRunningAmim
]]--
...
function RichLabel:createSprite_(parseArray)
	local spriteArray = {}
	-- cc.vv.dump(parseArray,"parseArray")

	-- for i=#parseArray,1,-1 do
	-- 	if not parseArray[i].textArray then
	-- 		self.isBreak = true
	-- 		table.remove(parseArray,i)
	-- 	end
	-- end
	if parseArray and #parseArray > 0 then
		for i, dic in ipairs(parseArray) do
			...
			if textArr and #textArr > 0 then --创建文字
				...
				for j, word in ipairs(textArr) do
					local label = display.newTTFLabel({text = word,
													font = fontName,
													size = fontSize})
