...
local MJLogic = class("MJJZLogic", MJLogic)
...
-- 判断是否出牌后可以听牌
function MJLogic.CanOutToTing(hand, group, wang, is13Yao)
    ...
    for i, v in ipairs(setCard) do
        ...
        if MJLogic.JudgeHu(new_data, group, wang, is13Yao) > 0 then
            return true
        end
    end
    return false
...
-- 判断能否听牌
function MJLogic.CanTing(hand, group, wang, is13Yao)
    wang = wang or MJLogic.FAKE_WANG
    local new_data = table.copyArray(hand)
    new_data[#new_data + 1] = wang
    if MJLogic.JudgeHu(new_data, group, wang, is13Yao) > 0 then
...
-- 判断是否可以接炮胡
function MJLogic.CanChiHu(hand, group_data, card, wang, is13Yao)
    local new_data = table.copyArray(hand)
    table.insert(new_data, card)
    local new_data2 = clone(group_data)
    if MJLogic.JudgeHu(new_data, new_data2, wang, is13Yao) > 0 then
        return true
    end
    return false
end
...
-- 判断自摸
function MJLogic.CanZimoHu(hand, group_data, wang, is13Yao)
    local new_data = table.copyArray(hand)
    local new_data2 = clone(group_data)
    if MJLogic.JudgeHu(new_data, new_data2, wang, is13Yao) > 0 then
        return true
...
-- 获取听牌列表
function MJLogic.CetTingCards(hand, group, wang, is13Yao)
    local ting_cards = {}
    local new_data = table.copyArray(hand)
    for _, card in ipairs(MJLogic.CARD_ALL) do
        new_data[#new_data + 1] = card
        local hu = MJLogic.JudgeHu(new_data, group, wang, is13Yao)
        if hu > 0 then
...
-- 砍胡(胡单张，包括夹中、单调、边夹)
function MJLogic.JudgeHu_KANHU(hand, wang, hu_card)
    local delCards = {}
...
function MJLogic.JudgeHuTypes(hand, group, wang, hu_card, is13Yao)
    local types = {}
    ...
    if is13Yao then
        ...
        if hu_type > 0 then
            ...
            types[#types + 1] = MJLogic.HU_MENQING
            return types
        end
    end
    ...
    if hu_type > 0 then
        ...
        if hu_type > 0 then
            ...
            -- 砍胡
            hu_type = MJLogic.JudgeHu_KANHU(hand, wang, hu_card)
            if hu_type > 0 then
...
function MJLogic.JudgeHu(hand, group, wang, is13Yao)
    -- 判断十三幺(不需要缺门前提)
    if is13Yao then
        local hu_type = MJLogic.JudgeHu_SSY(hand, wang)
        if hu_type > 0 then
