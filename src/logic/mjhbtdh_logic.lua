...
local MJLogic = class("MJHBTDHLogic", MJLogic)
...
-- @param group: 牌面上的碰牌
-- @param pass: 不要的牌
function MJLogic.GetCanGang(hand, group, pass, is_ting, wang, tingCards, isHPBXQM)
        ...
        if tingCards and #tingCards > 0 then -- 改变听口不能杠
            ...
            for _, gangCard in ipairs(gangCards) do
                ...
                for _, card in ipairs(tingCards) do
                    ...
                    local hu = MJLogic.JudgeHu(new_data, group, wang, isHPBXQM)
                    if hu > 0 then
            ...
            for _, gangCard in ipairs(gangCards) do
                ...
                if MJLogic.CanTing(new_data, group, wang, isHPBXQM) then
                    canGangCards[#canGangCards + 1] = gangCard
                end
            end
            return canGangCards, anGangCards, jiaGangCards
...
-- 判断是否可以杠指定的牌
function MJLogic.CanGang(hand, gangCard, is_ting, wang, tingCards, isHPBXQM)
    local hasKe = false
    ...
    if is_ting then
        if tingCards and #tingCards > 0 then -- 改变听口不能杠
            ...
            for _, card in ipairs(tingCards) do
                new_data[#new_data + 1] = card
                local hu = MJLogic.JudgeHu(new_data, {}, wang, isHPBXQM)
                if hu > 0 then
                    table.insert(tmpTingCards, card)
                end
                new_data[#new_data] = nil
            ...
            local new_data = clone(hand)
            MJLogic.delete_card(new_data, {gangCard, gangCard, gangCard})
            if MJLogic.CanTing(new_data, {}, wang, isHPBXQM) then
                return true
            end
        end
    end
...
-- 判断是否出牌后可以听牌
function MJLogic.CanOutToTing(hand, group, wang, isHPBXQM)
    ...
    for i, v in ipairs(setCard) do
        ...
        if MJLogic.JudgeHu(new_data, group, wang, isHPBXQM) > 0 then
            return true
        end
    end
    return false
...
-- 判断能否听牌
function MJLogic.CanTing(hand, group, wang, isHPBXQM)
    wang = wang or MJLogic.FAKE_WANG
    local new_data = table.copyArray(hand)
    new_data[#new_data + 1] = wang
    if MJLogic.JudgeHu(new_data, group, wang, isHPBXQM) > 0 then
...
function MJLogic.CanChiHu(hand, group_data, card, wang, isHPBXQM)
    local new_data = table.copyArray(hand)
    if card == wang then
    ...
    if MJLogic.JudgeHu(new_data, new_data2, wang, isHPBXQM) > 0 then
        return true
    end
    return false
end
...
-- 判断自摸
function MJLogic.CanZimoHu(hand, group_data, wang, isHPBXQM)
    local new_data = table.copyArray(hand)
    local new_data2 = clone(group_data)
    if MJLogic.JudgeHu(new_data, new_data2, wang, isHPBXQM) > 0 then
        return true
...
-- 获取听牌列表
function MJLogic.CetTingCards(hand, group, wang, isHPBXQM)
    local ting_cards = {}
    local new_data = table.copyArray(hand)
...
function MJLogic.JudgeHu_Duidui(hand, wang, isHPBXQM)
    if #hand ~= 14 then
...
-- 清一色 (胡牌)
function MJLogic.JudgeHu_QINGYISE(hand, group, wang, isHPBXQM)
    if isHPBXQM and not MJLogic.isQueMen(hand, group, wang) then
        return MJLogic.HU_NONE
    end
    ...
    -- 胡牌
    if MJLogic.JudgeHu_Normal(hand, group, wang, isHPBXQM) == MJLogic.HU_NORMAL or MJLogic.JudgeHu_Duidui(hand, wang, isHPBXQM) then
...
function MJLogic.JudgeHuTypes(hand, group, hu_types, wang, isHPBXQM)
    local types = hu_types or {}

    local hu_type = MJLogic.JudgeHu_Duidui(hand, wang, isHPBXQM)
    if hu_type > 0 then
    ...
    hu_type = MJLogic.JudgeHu_QINGYISE(hand, group, wang, isHPBXQM)
    if hu_type  > 0 then
        table.insert(types, hu_type)
    end
...
function MJLogic.JudgeHu(hand, group, wang, isHPBXQM)
    local hu_type = 0

    hu_type = MJLogic.JudgeHu_Duidui(hand, wang, isHPBXQM)
    if hu_type > 0 then
        return hu_type
    end
