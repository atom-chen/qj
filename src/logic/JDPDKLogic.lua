...
PDKLogic.CT_THREE_LINE          = 8      --  飞机： 三顺。比如777888或444555
PDKLogic.CT_THREE_LINE_TAKE     = 9      --  飞机少带： 三顺+同数量一手牌。比如777888+34或444555+337
PDKLogic.CT_THREE_LINE_TAKE_TWO = 10     --  飞机带翅膀： 三顺+同数量一手牌。比如777888+34+56或444555+33+77
PDKLogic.CT_FOUR_TAKE_TWO       = 11     --  四带二：四条+2手牌。比如AAAA+79或9999+33
PDKLogic.CT_BOMB                = 12     --  炸弹
PDKLogic.CT_FOUR_TAKE_THREE     = 13     --  四带三：四条+3手牌。比如AAAA+789或9999+333
...
function PDKLogic:GetCardType(data, opts, is_last)
    ...
    if analyseResult.fourCount > 0 then
        ...
        -- 三带二
        if (analyseResult.fourCount == 1) and (count == 5) then return PDKLogic.CT_THREE_TAKE_TWO end
        ...
        -- 飞机带翅膀
        if lineNum * 5 == count then
            return PDKLogic.CT_THREE_LINE_TAKE_TWO
        end

        -- 飞机
        if is_last and (lineNum * 3 == count) then
            return PDKLogic.CT_THREE_LINE
        end

        -- 飞机少带
        if is_last and (lineNum*5 >= count) then
            return PDKLogic.CT_THREE_LINE_TAKE
        end
    ...
    if analyseResult.threeCount > 0 then
        ...
        if (analyseResult.threeCount == 1) and (count == 5) then
...
function PDKLogic:CompareCard(firstCards, nextCards, opts, is_last)
    ...
    if firstType == PDKLogic.CT_THREE_LINE and
        (nextType == PDKLogic.CT_THREE_LINE or
    ...
    elseif nextType == PDKLogic.CT_THREE_TAKE_TWO or
        nextType == PDKLogic.CT_THREE_LINE_TAKE_TWO or
        nextType == PDKLogic.CT_THREE_LINE
        then
        -- 分析扑克
        local firstResult = {
            fourCount      = 0,
...
function PDKLogic:SearchOutCard(handCardData, turnCardData, outCardResult, chooseShunzi, opts, is_last)
    ...
    local turnType = self:GetCardType(turnCardData, opts, true)
    ...
    elseif (turnType == PDKLogic.CT_THREE_TAKE_TWO or -- 三带二
            turnType == PDKLogic.CT_THREE_LINE_TAKE_TWO or -- 飞机翅膀
            turnType == PDKLogic.CT_THREE_LINE) and
                ...
                for j = cardCount - i + 1, cardCount - 2 do
                    ...
                    if ((self:GetLogicValue(cardData[j]) + lineCount) == handLogicValue) and
                        ...
                        if lineCount == turnLineCount then
                            ...
                            if turnType == PDKLogic.CT_THREE_LINE_TAKE_TWO or turnType == PDKLogic.CT_THREE_TAKE_TWO or
                                turnType == PDKLogic.CT_THREE_LINE then
                                    if turnType == PDKLogic.CT_THREE_LINE and not is_last then
                                        turnCardCount = turnCardCount/3*2 + turnCardCount
                                    end
                                -- 提取 from 单牌
